...
@section('content')
    <form action="{{ route('Juegos.Guardar') }}" id="frmDetalle" class="form-horizontal" method="post"
            ...
            <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12" id="menu-4buttons">
                <div style="height:30px;text-align:left;margin:0 auto;" class="mostrar-subbutton">
                    ...
                    <button id="btnSalir" class="button-submenu mostrar-subbutton elimina-boton-derecho" title="Nuevo"
                        onclick="window.location='{{ route('Juegos.Index') }}'">
                        <span>
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                class="bi bi-save2" viewBox="0 0 16 16" style="margin-top: -5px;">
                ...
                <h4 style="font-weight:600;">
                    ...
                    <span style="color:#a0a0a0;">
                        ...
                        <span style="color:#fff;">
                            <a onclick="LinkAtras('/CapEstimate')">Detalle </a>
                        </span>
                    </span>
                </h4>
            </div>
        ...
        <div class="table-container">
            ...
            <div class="table-border" style="float:left; margin-top:-15px;">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 col-xxs div-border form-bord"
                    ...
                    {{ csrf_field() }}
                    <input type="hidden" class="form-control" id="idold" name="idold" value="{{ $juego->id }}">
                    <div class="mb-3 ml-10 mr-10" style="margin-left: 10px; margin-right: 10px;">
                        <label for="id" class="form-label">Codigo</label>
                        <input type="text" required class="form-control{{ $juego->id == 0 ? '' : '-plaintext' }}"
                            id="id" name="id" value="{{ $juego->id == 0 ? '' : $juego->id }}"
                            {{ $juego->id == 0 ? '' : 'readonly' }}>
                    ...
                    <div class="mb-3" style="margin-left: 10px; margin-right: 10px;">
                        ...
                        <select class="form-select" aria-label="Default select example" name="codigocategoria"
                            id="codigocategoria">
                            @foreach ($categorias as $vloCategoria)
                                @if ($juego->categoria !== 0)
                                    @if ($juego->categoria === $vloCategoria->id)
                                        <option value="{{ $vloCategoria->id }}" selected>
                                            {{ $vloCategoria->nombre }}
                                        </option>
                                    @else
                                        <option value="{{ $vloCategoria->id }}">
                                            {{ $vloCategoria->nombre }}
                                        </option>
                                    @endif
                                @else
                    ...
                    <div class="d-flex">
                        <div class="float-left mb-3" style="margin-left: 10px; margin-right: 10px;">
                            <label for="foto">Foto del juego</label>
                            <input type="file" class="form-control-file" id="foto" name="foto"
                                accept="image/x-png,image/jpeg">
                        </div>
                        <div class="float-right mb-3" style="margin-left: 10px; margin-right: 10px;">
                            @if ($juego->rutafoto !== '')
                                {{-- <img style="width: 250px; height: 200px" src="{{ $juego->rutafoto }}" /> --}}
                                <img style="width: 250px; height: 200px" src="/images/{{ $juego->rutafoto }}" />
                            @endif
                        </div>
                    </div>
                </div>
